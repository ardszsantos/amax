...
func _process(delta):
	if is_clicking:
		...
		if click_timer >= click_timeout:
			is_clicking = false
			sprite.frame = 0 # Quando parar de clicar, todo mundo volta pro frame 0 (Idle)
...
func _on_input_event(_viewport, event, _shape_idx):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if total_frames > 1:
			# === REGRA DE LOOP DINÂMICA ===
			# Define qual frame o ciclo de clique recomeça quando chega no final.
			var loop_start = 1 # O padrão é voltar pro frame 1 (como o 67)
			
			match item.item_name:
				"67":
					loop_start = 1 # 1 -> 2 -> 1
				"Mewing":
					loop_start = 1 # (Se o Mewing também pular o idle, deixe 1)
				"Gloving", "Academia", "Hype Beast": 
					# Coloque aqui o nome exato dos itens que incluem o frame 0 no clique
					loop_start = 0 # 0 -> 1 -> 2 -> 0
...
func _on_item_changed(_new_index: int):
	# === EFEITO SCROLL TIKTOK CORRIGIDO ===
